...
impl IrBuilder {
    ...
    fn build_statement(
        ...
        match stmt {
            Statement::OpAssign(op_assign) => self.build_op_assign(ctx, block, op_assign),
            Statement::Return(ret) => self.build_return(ctx, block, ret),
        }
    }
    ...
    fn build_op_assign(
            ...
            match operand {
                ...
                Operand::Literal(lit) => {
                    let (ty, attr_val) = match lit {
                        ...
                        LiteralValue::Float(v) => (ctx.make_float_type(64), Attribute::Float(*v)),
                        LiteralValue::Bool(v) => (ctx.make_bool_type(), Attribute::Bool(*v)),
        ...
        let (dialect, _op_short) = op.op_name.split_once('.').unwrap_or(("core", &op.op_name));
    ...
    fn resolve_type_expr(
        ...
        ctx: &mut Context,
        ty: &TypeExpr,
    ) -> Result<lift_core::types::TypeId, String> {
        match ty {
            TypeExpr::Tensor(t) => {
                let shape: Vec<Dimension> = t
                    .shape
                    .iter()
                    .map(|d| match d {
            ...
            TypeExpr::Scalar(s) => match s.name.as_str() {
    ...
    fn convert_attr_value(&self, ctx: &mut Context, val: &AttrValue) -> Attribute {
        match val {
            ...
            AttrValue::Array(arr) => Attribute::Array(
                arr.iter()
                    .map(|v| self.convert_attr_value(ctx, v))
                    .collect(),
            ),
        }
    }
