...
fn main() {
    ...
    println!(
        "  [WRITE] examples/generated_optimize.lith ({} bytes)",
        config.len()
    );
...
fn emit_and_validate(name: &str, builder: ModelBuilder) {
    ...
    let has_quantum = ctx
        .ops
        .iter()
        .any(|(_, op)| ctx.strings.resolve(op.name).starts_with("quantum."));
...
fn build_resnet_block() -> ModelBuilder {
    ...
    ModelBuilder::new("resnet_block")
        ...
        .param("bn1_b", tensor_1d(64, f))
        .param("bn2_w", tensor_1d(64, f))
        .param("bn2_b", tensor_1d(64, f))
        .op(
            "tensor.conv2d",
            &["x", "w1"],
            "c1",
            tensor_4d(1, 64, 56, 56, f),
        )
        .op(
            "tensor.batchnorm",
            &["c1", "bn1_w", "bn1_b"],
...
fn format_flops(f: u64) -> String {
    if f >= 1_000_000_000_000 {
        format!("{:.2} TFLOP", f as f64 / 1e12)
    } else if f >= 1_000_000_000 {
        format!("{:.2} GFLOP", f as f64 / 1e9)
    } else if f >= 1_000_000 {
