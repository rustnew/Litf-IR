...
fn test_all_hybrid_ops_have_names() {
    ...
    for op in &ops {
        ...
        assert!(
            name.starts_with("hybrid."),
            "{:?} should start with hybrid.",
            op
        );
...
fn test_hybrid_gradient_and_variational_disjoint() {
    ...
    for op in &ops {
        ...
        assert!(
            !(op.is_gradient() && op.is_variational()),
            "{:?} is both gradient and variational",
            op
        );
    }
}
